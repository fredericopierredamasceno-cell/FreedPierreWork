...
import { projectsOfService, orphanProjects } from "../lib/services";
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="border border-border divide-y divide-border">
            {cms.services.map(svc => {
              const total = projectsOfService(allProjects, svc).length;
              if (!total) return null;
              return (
                <div key={svc.id} className="flex items-center justify-between px-3 py-2">
                  <span className="text-xs text-foreground font-light">{svc.title}{svc.active ? "" : " (inativo)"}</span>
                  <span className="font-mono text-[9px] text-muted-foreground">{total} item(s)</span>
                </div>
              );
            })}
            {orphanProjects(allProjects, cms.services).length > 0 && (
              <div className="flex items-center justify-between px-3 py-2">
                <span className="text-xs text-muted-foreground font-light">Sem serviço</span>
                <span className="font-mono text-[9px] text-muted-foreground">{orphanProjects(allProjects, cms.services).length} item(s)</span>
              </div>
            )}
          </div>
