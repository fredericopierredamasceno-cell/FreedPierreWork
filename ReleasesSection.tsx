 ...
 */
export function ReleasesSection({ releases, showAdmin, onAdd, onEdit, onToggleHide, onDelete }: {
  releases: CMSRelease[];
  showAdmin: boolean;
  onAdd?: () => void;
  ...
  return (
    <section id="plataformas" className="py-16 md:py-24 border-t border-border">
      <div className="max-w-6xl mx-auto px-5 md:px-6">
        <FadeIn><SectionLabel>Ouça nas plataformas</SectionLabel></FadeIn>
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-10 md:mb-12">
          <FadeIn delay={60}>
            <h2 className="text-4xl md:text-7xl font-black uppercase text-foreground leading-none" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
              Disponível<br /><span className="text-primary">nas plataformas</span>
            </h2>
          </FadeIn>
          {showAdmin && onAdd && (
            <FadeIn delay={100}>
        ...
        {visible.length === 0 ? (
          <div className="border border-dashed border-border py-16 flex flex-col items-center gap-3">
            <p className="font-mono text-xs text-muted-foreground tracking-widest uppercase text-center">Nenhum lançamento cadastrado ainda</p>
            {showAdmin && onAdd && (
              <button onClick={onAdd} className="flex items-center gap-2 bg-primary text-background px-5 py-2.5 font-bold text-xs tracking-widest uppercase mt-1">
                <Plus size={12} />Primeiro lançamento
              </button>
            )}
          </div>
        ) : (
